...
Hydra is built on OmegaConf. Key features: variable interpolation (`${model.name}`), built-in resolvers (`${oc.env:VAR}`), custom resolvers, MISSING values for required fields, struct mode, and merge operations. See `references/omegaconf.md` for the full reference.
